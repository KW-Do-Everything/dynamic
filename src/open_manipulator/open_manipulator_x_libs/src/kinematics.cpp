...
/*
포워드 키네매틱스 실행: 현재 조인트 각도에서 매니퓰레이터의 포워드 키네매틱스를 계산하여 현재의 엔드 이펙터 위치를 얻습니다.
*/
  solveForwardKinematics(&_manipulator);
  //////////////checking dx///////////////
/*

목표 위치와의 차이 계산: 목표 위치와 현재 위치 사이의 차이를 계산합니다. 이 차이는 목표 위치에 도달하기 위해 필요한 이동 벡터로 사용됩니다.
*/
...
/*
Jacobian 행렬 계산: 로봇의 현재 구성에 대한 Jacobian 행렬을 계산합니다. 이 행렬은 위치 변화에 대한 조인트 각도 변화의 관계를 나타냅니다.
...
/*
Singularity Robust Jacobian 계산: 기존 Jacobian에 조정 인자와 가중치를 적용하여 Singularity Robust Jacobian(SR Jacobian)을 계산합니다. 이는 특이점에서의 수치적 안정성을 개선하기 위한 것입니다.
...
//각도 변화 계산: SR Jacobian을 사용하여 필요한 조인트 각도 변화를 계산합니다. 이는 엔드 이펙터를 목표 위치로 이동시키기 위한 조인트의 회전량을 결정합니다.
...
/*
조인트 각도 업데이트 및 검증: 계산된 각도 변화를 현재 조인트 각도에 적용하고, 업데이트된 각도로 다시 포워드 키네매틱스를 실행하여 엔드 이펙터의 위치를 확인합니다.
*/
    for (int8_t index = 0; index < _manipulator.getDOF(); index++)
...
반복 수행: 에너지(목표 위치와의 거리 제곱)가 충분히 작아질 때까지 위의 단계를 반복합니다. 반복 동안 에너지가 줄어들지 않으면 각도 변경량을 조정하여 다시 시도합니다.
*/
